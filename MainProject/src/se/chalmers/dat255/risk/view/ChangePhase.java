...
public class ChangePhase extends AbstractView {
	...
	private String getText() {
		if ((model.getPhase() == se.chalmers.dat255.risk.model.PhaseHandler.Phase.F1)
				|| (model.getPhase() == se.chalmers.dat255.risk.model.PhaseHandler.Phase.FBuild)) {
			if (model.getBonusUnitsLeft() == 0) {
				return "Next";
			}
			return "Units: " + model.getBonusUnitsLeft();
		} else if (model.getPhase() == se.chalmers.dat255.risk.model.PhaseHandler.Phase.F2) {
			return "Next Phase";
		} else if (model.getPhase() == se.chalmers.dat255.risk.model.PhaseHandler.Phase.F3) {
			return "End Turn";
		}
